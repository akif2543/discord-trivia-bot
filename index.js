...
const playTrivia = async (message) => {
  ...
  try {
    ...
    message.channel.send(
      `**${
        question.text
      }**\nYou have 30 seconds to answer! Type ? before your response. The first person to answer correctly wins. This question is worth ${
        question.value
      } ${question.value == 1 ? "point" : "points"}.`
    );
...
const getLeader = async (message) => {
  ...
  try {
    ...
    let i = 1;
    json.forEach((player) => {
      board += `${i}. ${player.name}: ${player.totalScore}\n`;
      i++;
    });
    board = board.trim();
...
const getScore = async (message) => {
  try {
    ...
    return message.reply(`your score is ${player.totalScore}.`);
  } catch (err) {
    console.error(err);
  }
};
